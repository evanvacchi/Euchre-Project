...
class Euchre {
	...
	startRound() {
		...
		if (this.dealer === 0) {
			...
			for (var i = 0; i < 4; i++) {
				//computers 1-3 can say pick it up or pass...
				if (i === 3) {
					//player can pickup or pass
					...
					//if cpu passes then next cpu
					// setTimeout(() => {
					//   text.innerText = `Player ${i+1} passes!`
					// }, 1500)
					continue;
				} else {
					//computer has you pick up trump
	...
	async nextRound() {
		...
		console.log(`play leader is ${this.leader}`);

		text.innerText = 'Next Round!';
		await this.clearPlayedCards();
		/*
		setTimeout(() => {
			text.innerText = 'Next Round!';
		}, 2500);
		...
		*/
		...
		if (this.leader === 0) {
			//you lead
			this.handleChoice = 0;
			this.play(); //need to pause the remaining functions from running.
			/*
			sleep(2000);
			setTimeout(() => {
				// this.nextPlayer();
			...
			this.play(); //need to pause for player to play
			setTimeout(() => {
				this.cpuPlay();
				this.nextPlayer();
			}, 7500);
	...
	async handleNext(){
		if (this.roundValues.length === 4) {
			this.isRoundWinner();
			return;
		}
		switch(this.handleChoice){
			case 0:
				await this.playerZeroGoes();
				break;
			case 2:
				await this.playerTwoGoes();
				break;
			case 3:
				await this.playerThreeGoes();
				break;
		}
	}

	async playerZeroGoes() {
		await setTimeout(async () => {
			// this.nextPlayer();
			await this.cpuPlay();
			await this.nextPlayer();
	...
	async playerTwoGoes() {
		await this.cpuPlay();
		await setTimeout(async () => {
			await this.isRoundWinner();
		}, 1000);
	}

	async playerThreeGoes() {
		await setTimeout(async () => {
			await this.cpuPlay();
			await this.nextPlayer();
		}, 2500);
	...
	clearPlayedCards() {
		// removes played cards from every player's/cpu's hand(Array) after each turn
		console.log('clearing played cards');
		document.getElementById(this.p1PlayedId).style.display = 'none';
		this.roundPlayedCards.forEach(
			id => (document.getElementById(id).style.display = 'none')
		);

		this.playerCardsArray = this.playerCardsArray.filter(x => x.id !== this.p1PlayedId);

			//.splice(Number(this.p1PlayedId.slice(-1)) - 1, 1); //removes user's last played card from array
		// console.log(this.roundPlayedCards);
		//this.roundPlayedCards.pop(); //cp1slotcard was added to the end of array due to nextPlayer() call
		// console.log(this.roundPlayedCards);
		this.roundPlayedCards.forEach(selectedCard => {
			const cardsChoice = selectedCard.substring(0,3);
			switch(cardsChoice){
				case 'cp1':
					this.cpuOneCardsArray = this.cpuOneCardsArray.filter(x => x.id !== selectedCard);
					break;
				case 'cp2':
					this.cpuTwoCardsArray = this.cpuTwoCardsArray.filter(x => x.id !== selectedCard);
					break;
				case 'cp3':
					this.cpuThreeCardsArray = this.cpuThreeCardsArray.filter(x => x.id !== selectedCard);
					break;}

			/*
			var num = id.slice(2, 3);
			if (num == 1) {
				this.cpuOneCardsArray.splice(Number(id.slice(-1)) - 1, 1);
				// console.log(this.cpuOneCardsArray);
			} else if (num == 2) {
				this.cpuTwoCardsArray.splice(Number(id.slice(-1)) - 1, 1);
				// console.log(this.cpuTwoCardsArray);
			} else {
				this.cpuThreeCardsArray.splice(Number(id.slice(-1)) - 1, 1);
			}
			 */
		});
	}
	...
	secondBid() {
		//called when player clicks "pass" button or computer dealer passes after startRound
		...
		if (this.dealer === 0) {
			...
			for (var i = 0; i < 4; i++) {
				if (i === 3) {
					...
					break;
				} else if (this.cpuCallTrump() === false) {
					//computer passes again
	...
	cpuCallTrump() {
		//called during secondBid only
		// ***issue is that computer 0 (player is calling trump before cpu1 and player shouldn't be run in this)
		var arr = [];
		this.getPlayer().forEach(card =>
			arr.push(card.firstElementChild.dataset.value.slice(2).trim())
		);
		var counts = {};
		...
		if (counts[potential[0]] >= 3) {
			...
			//only computers use this function
			this.pass();
			return false;
		} else {
			this.pass();
	...
	screwTheDealer() {
		...
		return (this.roundTrump =
			possibleTrump[Math.floor(Math.random() * possibleTrump.length)]);
	}
	...
	orderUp() {
		...
		this.getPlayer().forEach(card =>
			arr.push(card.firstElementChild.dataset.value.slice(2).trim())
		); //puts suits of cards in array
		var counts = {};
		arr.forEach(function(x) {
			counts[x] = (counts[x] || 0) + 1;
		}); // counts all suits in array / in cpu's hand
		// console.log(counts);
		// console.log(Object.keys(counts).length + ' suited');
		var numTrump =
			counts[upcard.firstElementChild.dataset.value.slice(2).trim()]; //gets amount of potential trump minus the left...
		...
		if (numTrump >= 4) {
			...
			numTrump === 3 &&
			(this.isLeft() === true || this.isRight() === true) &&
			(this.isSideAce() === true || Object.keys(counts).length <= 3)
		) {
			text.innerText = `Player ${this.leader} orders it up!`;
			console.log(`Computer ${this.leader} Orders it up!!!`);
			console.log('trump is ' + this.roundTrump);
	...
	isPartner() {
		// should really be  isPARTNER DEALER
		if (
			this.leader === this.realdealer + 2 ||
			this.leader === this.realdealer - 2
		) {
	...
	cpuPickUp() {
		...
		for (var key in countingSuits) {
			if (countingSuits[key] === 1 && key !== this.roundTrump) {
				...
				if (
					this.getDealer()[index].firstElementChild.dataset.value !==
					'J ' + opposite[this.roundTrump] &&
					this.getDealer()
						[index].firstElementChild.dataset.value.slice(0, 2)
						.trim() !== ('A' || 'K')
				) {
					...
					console.log(
						`Computer ${
							this.realdealer
						} picks it up, discards, & short suit themself`
					);
					setTimeout(() => {
				...
				//discard low card as long as it's not trump or left
				var holding = [];
				var lowvals = [];
				for (var i = 0; i < arr.length; i++) {
					if (arr[i] !== this.roundTrump) {
						...
						lowvals.push(
							CARD_VALUE_MAP[
								this.getDealer()
									[i].firstElementChild.dataset.value.slice(0, 2)
									.trim()
								]
						);
					}
	...
	//playHand() works when player has last play of hand i.e. cpu1 leads
	playHand() {
		this.isLeader();
		console.log(`leader is ${this.leader}`);
		setTimeout(() => {
	...
	cpuLead() {
		...
		if (this.isSideAce() === true) {
			var arr1 = [];
			this.getPlayer().forEach(card =>
				arr1.push(card.firstElementChild.dataset.value)
			);
			...
			document.getElementById(
				this.getPlayer()[index].id
			).style.gridRowStart = x;
			document.getElementById(
				this.getPlayer()[index].id
			).style.gridColumnStart = y;
			...
			this.roundPlayedCards.push(this.getPlayer()[index].id);
			// console.log(this.getPlayer()[index]);
			this.leadingSuit = this.getPlayer()
				[index].firstElementChild.dataset.value.slice(2)
				.trim();
			// console.log(this.leadingSuit);
			return this.leadingSuit;
		} else if (
			this.orderer === this.leader + 2 ||
			this.orderer === this.leader - 2
		) {
			//if your partner ordered up trump, lead lowest trump
			var arr2 = [];
			this.getPlayer().forEach(card =>
				arr2.push(card.firstElementChild.dataset.value)
			);
			...
			if (
				this.getPlayer()[0].firstElementChild.dataset.value ===
				'J ' + opposite[this.roundTrump]
			) {
				this.leftIsLead = true;
				...
				this.leadValue = 24;
				this.roundValues.push(this.leadValue);
				this.roundPlayedCards.push(this.getPlayer()[0].id);
				...
				this.getPlayer()[0]
					.firstElementChild.dataset.value.slice(2)
					.trim() === this.roundTrump
			) {
				document.getElementById(this.getPlayer()[0].id).style.gridRowStart = x;
				document.getElementById(
					this.getPlayer()[0].id
				...
				this.leadingSuit = this.getPlayer()[0]
					.firstElementChild.dataset.value.slice(2)
					.trim();
				...
				this.leadValue =
					TRUMP_VALUE_MAP[
						this.getPlayer()[0]
							.firstElementChild.dataset.value.slice(0, 2)
							.trim()
						];
				this.roundValues.push(this.leadValue);
				this.roundPlayedCards.push(this.getPlayer()[0].id);
	...
	cpuPlay() {
		...
		var arr = [];
		var indexes = [];
		const info = this.getPlayer();

		this.getPlayer().forEach(card => {
			const cardBits = card.firstElementChild.dataset.value.split(' ');
			arr.push({suit: cardBits[1], number: cardBits[0], id: card.id});
		...
		var roundValue = CARD_VALUE_MAP[randomCard.number];

		this.roundValues.push(roundValue);
		this.roundPlayedCards.push(randomCard.id);

		document.getElementById(randomCard.id).style.gridRowStart = x;
		document.getElementById(randomCard.id).style.gridColumnStart = y;
		...
		} else if (this.leadingSuit === leftSuit && this.isLeft() === true) {
			// prevents left from being played as its suit
			if (indexes.length === 1) {
				console.log(`Computer ${this.leader} doesn't have to follow suit`); //take with lowest trump card if partner doesn't have it won
				var arr3 = [];
				var suits = ['♠', '♣', '♥', '♦'];
				this.getPlayer().forEach(card =>
					arr3.push(card.firstElementChild.dataset.value.slice(2).trim())
				);
				var countingSuits = {};
				...
				if (countingSuits.hasOwnProperty(this.roundTrump) === true) {
					if (countingSuits[this.roundTrump] === 1) {
						...
						console.log(
							`Computer ${this.leader} throws ${lowtrump} and trumps!`
						);
						this.roundValues.push(lowtrump);
						this.roundPlayedCards.push(this.getPlayer()[index].id);
						return;
					} else if (countingSuits[this.roundTrump] >= 2) {
						//if 2 or more trump in hand scans them and plays lowest
						var holding = [];
						var trumpvalues = [];
						...
						var lowtrump =
							TRUMP_VALUE_MAP[
								this.getPlayer()
									[realIndex].firstElementChild.dataset.value.slice(0, 2)
									.trim()
								];
						document.getElementById(
							this.getPlayer()[realIndex].id
						).style.gridRowStart = x;
						document.getElementById(
							this.getPlayer()[realIndex].id
						).style.gridColumnStart = y;
						console.log(
							`Computer ${
								this.leader
							} throws lowest of their trump ${lowtrump} and trumps!`
						);
						this.roundValues.push(lowtrump);
						this.roundPlayedCards.push(this.getPlayer()[realIndex].id);
						return;
					}
				} else {
					//two suited not lead suit or trump
					var offsuitvalues = [];
					for (var i = 0; i < arr3.length; i++) {
						offsuitvalues.push(
							CARD_VALUE_MAP[
		...
		if (indexes.length >= 1) {
			...
			if (newArr.length === 1) {
				//if the cpu only has one of that suit they play it
				document.getElementById(
					this.getPlayer()[indexes[0]].id
				).style.gridRowStart = x;
				document.getElementById(
					this.getPlayer()[indexes[0]].id
				).style.gridColumnStart = y;
				...
				this.roundPlayedCards.push(this.getPlayer()[indexes[0]].id);
				return;
			} else if (newArr.length > 1) {
				//need to fix over trumping partner's trump lead if they have it won situation
				...
				document.getElementById(
					this.getPlayer()[indexes[index]].id
				).style.gridRowStart = x;
				document.getElementById(
					this.getPlayer()[indexes[index]].id
				).style.gridColumnStart = y;
				console.log(
					`Computer ${this.leader} follows suit with high card ${highCard}!`
				);
				this.roundValues.push(highCard);
				this.roundPlayedCards.push(this.getPlayer()[indexes[index]].id);
				return;
			}
		} else {
			console.log(`Computer ${this.leader} doesn't have to follow suit`);
			if (
				this.roundValues.length === 1 &&
				this.leadingSuit === this.roundTrump
			) {
				...
				for (var key in countingSuits) {
					...
					if (countingSuits[key] === 1) {
						...
						if (lowcard <= 12) {
							...
							console.log(
								`Computer ${
									this.leader
								} throws ${lowcard} and short suits themself!`
							);
					...
					//else if two suited throw lower of the two cards
						...
						var c1value =
							CARD_VALUE_MAP[
								this.getPlayer()
									[firstCard].firstElementChild.dataset.value.slice(0, 2)
									.trim()
								];
						var c2value =
							CARD_VALUE_MAP[
								this.getPlayer()
									[secondCard].firstElementChild.dataset.value.slice(0, 2)
									.trim()
								];
						if (c1value < c2value) {
							document.getElementById(
								this.getPlayer()[firstCard].id
							).style.gridRowStart = x;
							document.getElementById(
								this.getPlayer()[firstCard].id
							).style.gridColumnStart = y;
							...
							this.roundPlayedCards.push(this.getPlayer()[firstCard].id);
							return;
						} else {
							document.getElementById(
								this.getPlayer()[secondCard].id
							).style.gridRowStart = x;
							document.getElementById(
								this.getPlayer()[secondCard].id
							).style.gridColumnStart = y;
							...
							this.roundPlayedCards.push(this.getPlayer()[secondCard].id);
							return;
						}
						...
						var c3value =
							CARD_VALUE_MAP[
								this.getPlayer()
									[thirdCard].firstElementChild.dataset.value.slice(0, 2)
									.trim()
								];
						if (c1value < c2value && c1value < c3value) {
							...
							console.log(
								`Computer ${this.leader} throws lower of three cards`
							);
							this.roundValues.push(c1value);
							this.roundPlayedCards.push(this.getPlayer()[firstCard].id);
							return;
							...
							this.roundPlayedCards.push(this.getPlayer()[thirdCard].id);
							return;
						}
					} else {
						// the below logic is bad, just random filler for now
						for (var i = 0; i < arr3.length; i++) {
							document.getElementById(
								this.getPlayer()[i].id
							).style.gridRowStart = x;
							...
							this.roundPlayedCards.push(this.getPlayer()[i].id);
							return;
						}
					}
				}
				...
				this.roundValues.length === 2 &&
				this.roundValues[0] > this.roundValues[1]
			) {
				...
				if (
					countingSuits.hasOwnProperty(this.roundTrump) === true &&
					this.roundValues[0] <= 12
				) {
					if (countingSuits[this.roundTrump] === 1) {
						var index = arr3.indexOf(this.roundTrump);
						var lowtrump =
					...
					for (var key in countingSuits) {
						//there is an issue where left is not accounted for
						if (
							countingSuits[key] === 1 &&
							countingSuits[key] !== this.roundTrump
						) {
							var a = arr3.indexOf(key);
							var avalue =
								CARD_VALUE_MAP[
									this.getPlayer()
										[a].firstElementChild.dataset.value.slice(0, 2)
										.trim()
									];
							if (avalue <= 12) {
								document.getElementById(
								...
								console.log(
									`Computer ${
										this.leader
									} throws ${avalue} off since partner is taking trick!`
								);
								this.roundValues.push(avalue);
								this.roundPlayedCards.push(this.getPlayer()[a].id);
								return;
							}
						} else if (countingSuits[key] === 2) {
							function getKeyByValue(object, value) {
							...
							if (c1value < c2value) {
								...
								console.log(
									`Computer ${this.leader} throws lower of two cards`
								);
								this.roundValues.push(c1value);
								this.roundPlayedCards.push(this.getPlayer()[firstCard].id);
								return;
				...
				this.roundValues.length === 3 &&
				this.roundValues[1] > (this.roundValues[0] && this.roundValues[2])
			) {
				//same rules as above
				var arr3 = [];
				var suits = ['♠', '♣', '♥', '♦'];
				...
				//bad logic below but want to just play a card everytime
				var arr3 = [];
				this.getPlayer().forEach(card =>
					arr3.push(card.firstElementChild.dataset.value.slice(2).trim())
				);
				if (arr3[0] === this.roundTrump) {
					...
					console.log(
						`Computer ${this.leader} arbitrarily throws first card in hand`
					);
	...
	isLeft() {
		...
		this.getPlayer().forEach(card =>
			anyJacks.push(card.firstElementChild.dataset.value)
		);
	...
	isSideAce() {
		...
		var trumpSuit = upcard.firstElementChild.dataset.value.slice(2).trim();
		this.getPlayer().forEach(card =>
			anyAces.push(card.firstElementChild.dataset.value)
		);
		offSuit = offSuit.filter(e => e !== trumpSuit); //removes trumpsuit from "offsuit array"
		this.getPlayer().forEach(card =>
			countSuits.push(card.firstElementChild.dataset.value.slice(2).trim())
		);
		var counts = {};
		...
		if (anyAces.includes('A ' + offSuit[0]) && counts[offSuit[0]] === 1) {
			...
			anyAces.includes('A ' + offSuit[1]) &&
			counts[offSuit[1]] === 1
		) {
			this.offSuit = offSuit[1];
			return true;
		} else if (
			anyAces.includes('A ' + offSuit[2]) &&
			counts[offSuit[2]] === 1
		) {
	...
	play() {
		...
		this.playerCardsArray.forEach(card =>
			arroop.push(card.firstElementChild.dataset.value.slice(2).trim())
		);
		...
		playerCardSlot1.addEventListener('click', () => {
			...
			if (
				this.playerCardsArray[0].firstElementChild.dataset.value
					.slice(2)
					.trim() === this.roundTrump
			) {
				var pcardvalue =
		...
		playerCardSlot2.addEventListener('click', () => {
			...
			//Need to account for if the user plays the left
			if (
				this.playerCardsArray[1].firstElementChild.dataset.value
					.slice(2)
					.trim() === this.roundTrump
			) {
				var pcardvalue =
		...
		playerCardSlot3.addEventListener('click', () => {
			...
			//Need to account for if the user plays the left
			if (
				this.playerCardsArray[2].firstElementChild.dataset.value
					.slice(2)
					.trim() === this.roundTrump
			) {
				var pcardvalue =
		...
		playerCardSlot4.addEventListener('click', () => {
			...
			//Need to account for if the user plays the left
			if (
				this.playerCardsArray[3].firstElementChild.dataset.value
					.slice(2)
					.trim() === this.roundTrump
			) {
				var pcardvalue =
		...
		playerCardSlot5.addEventListener('click', () => {
			...
			//Need to account for if the user plays the left
			if (
				this.playerCardsArray[4].firstElementChild.dataset.value
					.slice(2)
					.trim() === this.roundTrump
			) {
				var pcardvalue =
	...
	playerPickUp() {
		...
		upcard.addEventListener('click', () => this.clickHandlerX(), {
			once: true
		});
	}
	...
	clickHandler(slot) {
		...
		newDiv.classList.add(
			'card',
			upcard.firstElementChild.className.split(' ')[1]
		);
		...
		this.slots.forEach((slot, position) =>
			slot.removeEventListener('click', this.handlers[position])
		);
		this.stopAnimation();
	...
	playerDiscard() {
		...
		this.slots.forEach((slot, position) =>
			slot.addEventListener('click', this.handlers[position], { once: true })
		);
		return true;
	}
